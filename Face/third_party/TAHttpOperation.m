...
@implementation TAHttpOperation
...
+(TAHttpOperation*)httpOperationWithTimeoutInterval : (double)dblTimeout{
    TAHttpOperation* newComm = [[TAHttpOperation alloc]initWithTimeoutInterval:dblTimeout];
    return newComm;
}
...
-(void)checkHttpTimeout:(NSTimer*)timer{
	...
	if(block != nil) {
		block(self, TAHttpOperationTimeout);
	}
}
...
-(void)execCreateConnectDidFail{// it's on main thread now
	...
	if(self.failCB){
	    self.failCB(self, TAHttpOperationFailToCreateConnect);
    }
...
-(void)execConnectDidFail{// it's on main thread now
	...
	if(self.failCB){
	    self.failCB(self, TAHttpOperationServerNotFound);
    }
	[self completeOperation];
}
...
+(NSString*)descriptionOfError: (TAHttpOperationError) error{
	switch (error) {
		case TAHttpOperationFailToCreateConnect:
			return @"无法连接对象";
			
		case TAHttpOperationServerNotFound:
			return @"未找到服务器";
			
		case TAHttpOperationTimeout:
			return @"连接超时";
			
		case TAHttpOperationServerUnknown:
			return @"服务器发生未知错误";
...
@implementation TAHttpOperation(NSURLConnectionDelegate)
#pragma mark -
